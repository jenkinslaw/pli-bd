...
(function() {
  ...
  pli_bd.auther_prepend = "_auther_prepend_";
  pli_bd.title_selector = "td.toc-description:not(:first):not(:last)";
  pli_bd.auther_selector = "td.toc-faculty:has(a)";

  // Tag all title text.
  pli_bd.titles = $(pli_bd.title_selector).prepend(pli_bd.title_prepend);
  // Tag all auther text.
  pli_bd.authers = $(pli_bd.auther_selector).prepend(pli_bd.auther_prepend);

  // Select all tagged text.
  pli_bd.OCLC_content = $(pli_bd.title_selector + ", " + pli_bd.auther_selector).text();

  // Prepend OCLC formated text to table for easy access.
  $(".Description").prepend(
    "<div class=\"greasemonkey\" bgcolor=\"green\">" + get_OCLC(pli_bd.OCLC_content) + "</div>");
